...
window.addEventListener( 'load', () => {
   ...
   //         base_room_container.innerHTML += `<li id="room-list-"><a href="#">Note</a></li>`
            for(var i=0;i<data.length;i++){ 

                // add rooms in the dropdown menu
                base_room_container.innerHTML += `<li id="room-list-${data[i]}"><a href="#${data[i]}">${data[i]}</a></li>`;

                // add corresponding chat sections to each room
                base_container.innerHTML += createRoomChatHolder(data[i]);
